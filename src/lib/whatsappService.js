...
const DEFAULT_CLIENT_ID = 'el-rahma-client';
...
export function getCachedStatus() {
  ...
  if (
    ...
    state.lifecyclePhase = 'WAITING_FOR_PHONE';
    state.lastInitError = null;
  }
  ...
  return {
    ...
    qr: null,
    qrAvailable: false,
    qrPending: state.status === 'INITIALIZING' && Boolean(state.pairingPhoneNumber) && !codeFresh,
    qrError: state.lastInitError,
...
async function waitForFreshPairingCode(cleanedNumber, timeoutMs = 60000) {
  const startedAt = Date.now();

  while (Date.now() - startedAt < timeoutMs) {
    const sameNumber = state.pairingPhoneNumber === cleanedNumber;
    if (sameNumber && state.pairingCode && isPairingCodeFresh(state.pairingCodeIssuedAt)) {
    ...
    if (state.status === 'PAIRING_BLOCKED') {
      const error = new Error(state.lastInitError || 'WhatsApp pairing is temporarily blocked.');
      error.statusCode = 429;
      error.retryAfterSeconds = getPairingRetryAfterSeconds(state.pairingBlockedUntil);
      throw error;
    }

    if (['ERROR', 'AUTH_FAILURE'].includes(state.status)) {
      const error = new Error(state.lastInitError || 'Failed to generate WhatsApp pairing code.');
  ...
  const error = new Error('Timeout requesting pairing code');
...
export async function requestPairingCode(phoneNumber) {
  ...
  await runLifecycleOperation('phone pairing', async () => {
    if (['CONNECTED', 'AUTHENTICATED'].includes(state.status)) {
      const error = new Error('WhatsApp is already connected.');
      error.statusCode = 409;
      throw error;
  ...
  return waitForFreshPairingCode(cleanedNumber);
}
