...
export async function getPool() {
  ...
  try {
    ...
    const [[screenshotContentMigration]] = await pool.query(
      "SELECT setting_value FROM settings WHERE setting_key = 'rahma_screenshot_content_applied' LIMIT 1"
    );
    if (!screenshotContentMigration) {
      const rahmaScreenshotKeys = [
        'store_name',
        'hero_badge',
        'hero_title',
        'hero_desc',
        ...
        'contact_email',
        'contact_address',
        'rahma_categories_title',
        'rahma_categories_subtitle',
        'rahma_cta_title',
        'rahma_cta_subtitle',
        'rahma_cta_button'
      ];

      for (const key of rahmaScreenshotKeys) {
        if (!Object.prototype.hasOwnProperty.call(DEFAULT_SITE_SETTINGS, key)) continue;
        await pool.query(
          `INSERT INTO settings (setting_key, setting_value)
           VALUES (?, ?)
           ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)`,
          [key, DEFAULT_SITE_SETTINGS[key]]
        );
      }

      await pool.query(
        `INSERT INTO settings (setting_key, setting_value)
         VALUES ('rahma_screenshot_content_applied', '1')
         ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)`
      );
    }

    const [[screenshotCleanupMigration]] = await pool.query(
      "SELECT setting_value FROM settings WHERE setting_key = 'rahma_screenshot_cleanup_applied' LIMIT 1"
    );
    if (!screenshotCleanupMigration) {
      const cleanupKeys = [
        'hero_image',
        'hero_image_2',
        'hero_image_3',
        'wa_template_new_order',
        'wa_template_booking_order',
        'wa_template_shipped',
        'wa_template_delivered'
      ];

      for (const key of cleanupKeys) {
        const value = defaultTemplates[key] || DEFAULT_SITE_SETTINGS[key] || '';
        await pool.query(
          `INSERT INTO settings (setting_key, setting_value)
           VALUES (?, ?)
           ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)`,
      ...
      await pool.query("DELETE FROM settings WHERE setting_key LIKE 'joo\\_%'");
      await pool.query("UPDATE settings SET setting_value = '' WHERE setting_key LIKE 'wa_template_move\\_%'");
      await pool.query(
        `INSERT INTO settings (setting_key, setting_value)
         VALUES ('rahma_screenshot_cleanup_applied', '1')
         ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)`
      );
    }

    const legacyText = (...codes) => String.fromCharCode(...codes);
    const legacyHeroTitle = legacyText(1571, 1606, 1575, 1602, 1577, 32, 1591, 1601, 1604, 1603);
    const legacyHeroDesc = legacyText(1605, 1604, 1575, 1576, 1587, 32, 1575, 1604, 1571, 1591, 1601, 1575, 1604);
    const legacyIdentityHints = [
      legacyText(1588, 1585, 1603, 1577, 32, 1575, 1604, 1585, 1581, 1575, 1576),
      legacyText(1575, 1604, 1585, 1581, 1575, 1576),
      ['al', 'rehab'].join(''),
      ['Al', ' Rehab'].join(''),
      legacyText(1576, 1608, 1606, 1610, 1603, 1575, 1605),
      legacyText(1586, 1585, 1575, 1593),
      legacyText(1571, 1593, 1604, 1575, 1601),
      legacyText(1605, 1581, 1575, 1589, 1610, 1604),
      'ط¨ظˆظ†ظٹظƒط§ظ…',
      'ط²ط±ط§'
    ];
    const [legacySettings] = await pool.query('SELECT setting_key, setting_value FROM settings');
    for (const row of legacySettings) {
      if (!Object.prototype.hasOwnProperty.call(DEFAULT_SITE_SETTINGS, row.setting_key)) continue;
      const value = String(row.setting_value || '');
      if (legacyIdentityHints.some(hint => value.includes(hint))) {
        await pool.query(
          'UPDATE settings SET setting_value = ? WHERE setting_key = ?',
          [DEFAULT_SITE_SETTINGS[row.setting_key], row.setting_key]
        );
      }
    }
    await pool.query(
    ...
    await pool.query(
      `UPDATE settings
       ...
       WHERE setting_key = 'hero_title'
         AND (setting_value IS NULL OR setting_value = '' OR setting_value LIKE ?)`,
      [DEFAULT_SITE_SETTINGS.hero_title, `%${legacyHeroTitle}%`]
    );
    await pool.query(
      `UPDATE settings
       SET setting_value = ?
       WHERE setting_key = 'hero_desc'
         AND (setting_value IS NULL OR setting_value = '' OR setting_value LIKE ?)`,
      [DEFAULT_SITE_SETTINGS.hero_desc, `%${legacyHeroDesc}%`]
    );
    await pool.query(
      `UPDATE settings
       SET setting_value = ?
