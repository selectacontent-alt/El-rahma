...
const ADMIN_TABS = ['dashboard', 'notifications', 'orders', 'customers', 'products', 'categories', 'coupons', 'settings', 'shipping', 'shipping_management', 'testimonials', 'media', 'whatsapp', 'users', 'catalog_translations', 'messages', 'about_select'];
...
const AdminPage = ({ setCurrentPage, user, setAuth }) => {
  ...
  const handleRequestPairingCode = async (e) => {
    ...
    try {
      ...
      setWaPairingCode(data.code || data.pairing?.code || '');
    } catch (err) {
    ...
    if (activeTab === 'whatsapp') {
      const fetchWaStatus = async () => {
        try {
          ...
          if (data.pairing?.code && data.pairing?.codeFresh !== false) {
            setWaPairingCode(data.pairing.code);
          }
          if (data.qr) {
            setWaQr(data.qr);
  ...
  const handleSettingsChange = (e) => {
    ...
    setSettings(prev => ({ ...prev, [name]: nextValue }));
  };
          ...
          {activeTab === 'settings' && (
            <div className="admin-panel" style={{ padding: '2rem', borderRadius: '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', overflowX: 'auto', paddingBottom: '0.5rem', borderBottom: '1px solid #f1f5f9' }}>
                {[
                  { id: 'rahma_home', label: 'هوية الرحمة' },
              ...
              <form onSubmit={handleSaveSettings} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                {/* SUBTAB: RAHMA HOME */}
                {settingsSubTab === 'rahma_home' && (
                  <div className="admin-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div>
                      <h4 style={{ fontSize: '1.25rem', color: 'var(--text-dark)', fontWeight: '900', marginBottom: '0.5rem' }}>محتوى قالب شركة الرحمة</h4>
                      <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', margin: 0 }}>كل الحقول هنا تظهر مباشرة في الصفحة الرئيسية الجديدة.</p>
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
                      {[
                        ['store_name', 'اسم المصنع'],
                        ['rahma_brand_subtitle', 'الوصف المختصر تحت الاسم'],
                        ['rahma_quote_button', 'زر عرض السعر'],
                        ['admin_whatsapp', 'رقم الهاتف الأول'],
                        ['support_whatsapp', 'رقم الهاتف الثاني'],
                        ['contact_phone_3', 'رقم الهاتف الثالث'],
                        ['contact_email', 'البريد الإلكتروني'],
                        ['contact_address', 'العنوان']
                      ].map(([name, label]) => (
                        <div className="admin-form-group" key={name}>
                          <label style={{ fontWeight: '700' }}>{label}</label>
                          <input type="text" name={name} value={settings[name] || ''} onChange={handleSettingsChange} className="admin-input" />
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{ background: '#f8fafc', padding: '1.2rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
                        {[
                          ['hero_badge', 'السطر الصغير'],
                          ['hero_title', 'العنوان الرئيسي'],
                          ['rahma_hero_primary_cta', 'زر المنتجات'],
                          ['rahma_hero_secondary_cta', 'زر الاستشارة']
                        ].map(([name, label]) => (
                          <div className="admin-form-group" key={name}>
                            <label style={{ fontWeight: '700' }}>{label}</label>
                            <input type="text" name={name} value={settings[name] || ''} onChange={handleSettingsChange} className="admin-input" />
                      ...
                      <div className="admin-form-group" style={{ marginTop: '1rem' }}>
                        <label style={{ fontWeight: '700' }}>وصف الهيرو</label>
                        <textarea name="hero_desc" value={settings.hero_desc || ''} onChange={handleSettingsChange} className="admin-input" rows="3" style={{ resize: 'vertical' }} />
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginTop: '1rem' }}>
                        {[
                          ['rahma_trust_1', 'نقطة ثقة 1'],
                          ['rahma_trust_2', 'نقطة ثقة 2'],
                          ['rahma_trust_3', 'نقطة ثقة 3']
                        ].map(([name, label]) => (
                          <div className="admin-form-group" key={name}>
                            <label style={{ fontWeight: '700' }}>{label}</label>
                            <input type="text" name={name} value={settings[name] || ''} onChange={handleSettingsChange} className="admin-input" />
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
                      <div style={{ background: '#f8fafc', padding: '1.2rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                        <h5 style={{ margin: '0 0 1rem', color: 'var(--primary-color)', fontWeight: '900' }}>منظومة التأريض</h5>
                        <div className="admin-form-group" style={{ marginBottom: '1rem' }}>
                          <label style={{ fontWeight: '700' }}>العنوان</label>
                          <input type="text" name="rahma_flow_title" value={settings.rahma_flow_title || ''} onChange={handleSettingsChange} className="admin-input" />
                        </div>
                        <div className="admin-form-group">
                          <label style={{ fontWeight: '700' }}>الوصف</label>
                          <textarea name="rahma_flow_subtitle" value={settings.rahma_flow_subtitle || ''} onChange={handleSettingsChange} className="admin-input" rows="3" />
                        </div>
                      </div>
                    ...
                    <div style={{ background: '#f8fafc', padding: '1.2rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                      <h5 style={{ margin: '0 0 1rem', color: 'var(--primary-color)', fontWeight: '900' }}>لماذا شركة الرحمة؟</h5>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
                        <div className="admin-form-group">
                          <label style={{ fontWeight: '700' }}>عنوان القسم</label>
                          <input type="text" name="why_title" value={settings.why_title || ''} onChange={handleSettingsChange} className="admin-input" />
                        </div>
                        <div className="admin-form-group">
                          <label style={{ fontWeight: '700' }}>وصف القسم</label>
                          <textarea name="why_desc" value={settings.why_desc || ''} onChange={handleSettingsChange} className="admin-input" rows="2" />
                        </div>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
                        {[1, 2, 3, 4, 5].map(num => (
                          <div key={num} style={{ background: '#fff', padding: '1rem', borderRadius: '14px', border: '1px solid #e2e8f0' }}>
                            <h6 style={{ margin: '0 0 0.8rem', fontWeight: '900', color: '#334155' }}>ميزة {num}</h6>
                            <input type="text" name={`feature_title_${num}`} value={settings[`feature_title_${num}`] || ''} onChange={handleSettingsChange} className="admin-input" placeholder="العنوان" style={{ marginBottom: '0.7rem' }} />
                            <textarea name={`feature_desc_${num}`} value={settings[`feature_desc_${num}`] || ''} onChange={handleSettingsChange} className="admin-input" rows="2" placeholder="الوصف" />
                          </div>
                        ))}
                      </div>
                    </div>
                        ...
                        {[
                          ...
                          <div key={item.key} style={{ background: '#fff', padding: '1rem', borderRadius: '14px', border: '1px solid #e2e8f0' }}>
                            <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.8rem', marginBottom: '0.9rem', cursor: 'pointer', fontWeight: '900', color: '#334155' }}>
                              ...
                              <input type="checkbox" name={`${item.key}_enabled`} checked={settings[`${item.key}_enabled`] !== 'false'} onChange={handleSettingsChange} />
                            </label>
                            <input type="text" name={`${item.key}_title`} value={settings[`${item.key}_title`] || ''} onChange={handleSettingsChange} className="admin-input" placeholder="العنوان" style={{ marginBottom: '0.7rem' }} />
                            <textarea name={`${item.key}_subtitle`} value={settings[`${item.key}_subtitle`] || ''} onChange={handleSettingsChange} className="admin-input" rows="2" placeholder="الوصف" style={{ marginBottom: '0.7rem' }} />
                            <input type="text" name={`${item.key}_button`} value={settings[`${item.key}_button`] || item.fallback} onChange={handleSettingsChange} className="admin-input" placeholder="نص زر صفحة المنتجات" />
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div style={{ background: '#f8fafc', padding: '1.2rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                      <h5 style={{ margin: '0 0 1rem', color: 'var(--primary-color)', fontWeight: '900' }}>قسم التواصل النهائي</h5>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
                        <div className="admin-form-group">
                          <label style={{ fontWeight: '700' }}>العنوان</label>
                          <input type="text" name="rahma_cta_title" value={settings.rahma_cta_title || ''} onChange={handleSettingsChange} className="admin-input" />
                        </div>
                        <div className="admin-form-group">
                          <label style={{ fontWeight: '700' }}>نص الزر</label>
                          <input type="text" name="rahma_cta_button" value={settings.rahma_cta_button || ''} onChange={handleSettingsChange} className="admin-input" />
                        </div>
                        <div className="admin-form-group">
                          <label style={{ fontWeight: '700' }}>وصف القسم</label>
                          <textarea name="rahma_cta_subtitle" value={settings.rahma_cta_subtitle || ''} onChange={handleSettingsChange} className="admin-input" rows="2" />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {settingsSubTab === 'about' && (
                  <div className="admin-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <div className="admin-form-group">
                      <label style={{ fontWeight: '700', fontSize: '0.9rem' }}>صورة قسم عن مصنع الرحمة</label>
                      <label className="admin-upload-box" style={{ minHeight: '120px', marginTop: '0.5rem' }}>
                  ...
                  <div className="admin-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    <div>
                      ...
                      <div className="admin-form-group" style={{ marginTop: '1.2rem' }}>
                        <label style={{ fontWeight: '700' }}>وصف قسم لماذا مصنع الرحمة</label>
                        <textarea name="why_desc" value={settings.why_desc || ''} onChange={handleSettingsChange} className="admin-input" rows="3" placeholder="اكتب وصفاً مختصراً يظهر أسفل عنوان قسم لماذا تختار مصنع الرحمة" style={{ resize: 'vertical' }}></textarea>
                      </div>
                    </div>
                ...
                {settingsSubTab === 'whatsapp' && (
                  <div className="admin-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    <div className="admin-form-group" style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                      <label style={{ fontWeight: '800', marginBottom: '1rem', display: 'block', color: 'var(--primary-color)' }}>قالب رسالة تأكيد طلب التوريد</label>
                      <textarea name="wa_template_booking_order" value={settings.wa_template_booking_order || ''} onChange={handleSettingsChange} className="admin-input" rows="9" placeholder="مرحباً {name}،&#10;تم استلام طلب التوريد الخاص بك بنجاح (طلب رقم #{order_id}).&#10;&#10;تفاصيل الطلب:&#10;- الكمية: {quantity}&#10;المنتجات:&#10;{items}&#10;&#10;الإجمالي: {total} ج.م&#10;&#10;سيتم التواصل معك قريباً لتأكيد موعد التجهيز والتسليم. شكراً لاختيارك مصنع الرحمة!" style={{ resize: 'vertical', padding: '1rem', lineHeight: '1.6' }}></textarea>
            ...
            <div className="admin-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
              ...
              <form onSubmit={handleSaveSettings} className="admin-panel">
                ...
                <div style={{ display: 'grid', gap: '1.5rem' }}>
                  ...
                  <div className="admin-form-group">
                    <label>قالب رسالة تأكيد طلب التوريد</label>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                  ...
                  <div style={{ background: '#f8fafc', padding: '2rem', borderRadius: '20px', border: '1px solid #e2e8f0' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: settings.bosta_enabled === 'true' ? '1.5rem' : '0' }}>
                      <h4 style={{ margin: 0, display: 'flex', alignItems: 'center' }}>
                        <img src="/bosta-logo.png?v=20260717" alt="Bosta Courier" style={{ height: '32px', maxWidth: '140px', objectFit: 'contain', display: 'block' }} />
                      </h4>
                      <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', gap: '0.5rem' }}>
                  ...
                  <div style={{ background: '#f8fafc', padding: '2rem', borderRadius: '20px', border: '1px solid #e2e8f0' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: settings.aramex_enabled === 'true' ? '1.5rem' : '0' }}>
                      <h4 style={{ margin: 0, display: 'flex', alignItems: 'center' }}>
                        <img src="/aramex-logo.png?v=20260717" alt="Aramex Courier" style={{ height: '32px', maxWidth: '140px', objectFit: 'contain', display: 'block' }} />
                      </h4>
                      <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', gap: '0.5rem' }}>
              ...
              <div style={{ background: 'linear-gradient(135deg, #9d027c 0%, #ffbc01 100%)', padding: '2rem', borderRadius: '20px', color: 'white', display: 'flex', flexWrap: 'wrap', gap: '2rem', justifyContent: 'space-around', alignItems: 'center', boxShadow: '0 10px 30px rgba(157, 2, 124, 0.2)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <a href="tel:01097041523" style={{ margin: 0, fontWeight: 'bold', fontSize: '1.2rem', direction: 'ltr', color: 'inherit', textDecoration: 'none', display: 'block' }}>01097041523</a>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
