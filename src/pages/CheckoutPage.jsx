...
import GroundingSeoArticle from '../components/GroundingSeoArticle';
import { getGovernorateLabel, getGovernorateOptions } from '../lib/localizedGovernorates';
...
const CheckoutPage = ({ cartItems = [], clearCart, setCurrentPage, removeFromCart, updateCartQuantity, customerAuth, setCustomerAuth }) => {
  ...
  const money = (value) => `${Number(value || 0).toLocaleString(language === 'ar' ? 'ar-EG' : 'en-US')} ${t('checkout.currency')}`;
  const hasAccountDeliveryData = hasSavedDeliveryData(savedCustomerProfile);
  const fallbackGovernorates = getGovernorateOptions(t);
  const shippingOptions = shippingRates.length > 0
    ? shippingRates.map((rate) => ({
      value: rate.gov,
      label: getGovernorateLabel(rate.gov, t),
      price: rate.price
    }))
    : fallbackGovernorates.map((option) => ({
      value: option.value,
      label: option.label,
      price: 0
    }));
                    ...
                    <select required value={orderForm.governorate} onChange={event => updateForm('governorate', event.target.value)}>
                      ...
                      {shippingOptions.map((option) => (
                        <option key={option.value} value={option.value}>
                          {option.label} {Number(option.price) > 0 ? `(+${option.price} ${t('checkout.currency')})` : ''}
                        </option>
                      ))}
                    </select>
                  </div>
        ...
        <div className="checkout-confirm-overlay">
          <section className="checkout-confirm-dialog">
            ...
            <div className="checkout-confirm-summary">
              ...
              <div><span>{t('checkoutNew.confirmGovernorate')}</span><b>{getGovernorateLabel(orderForm.governorate, t)}</b></div>
