      ...
      <div className="premium-card-image-wrap">
        ...
        {currentVariantStock <= 0 && (
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 10 }}>
            <span className="product-stock-status out-of-stock">{t('products.outOfStock')}</span>
          </div>
        )}

        {currentVariantStock > 0 && currentVariantStock <= 10 && (
          <div style={{ position: 'absolute', bottom: '10px', right: '10px', zIndex: 5 }}>
            <span className="product-stock-status low-stock">{t('products.lowStock', { stock: currentVariantStock })}</span>
          </div>
        )}
          ...
          {currentVariantStock <= 0 ? (
            ...
            shouldOpenProductFirst ? (
              ...
              <button 
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  const text = encodeURIComponent(t('products.quoteRequestText', { title: product.title }));
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center' }}>
                            {descImagesArr.map((imgUrl, idx) => (
                              <div key={idx} className="description-image-wrapper" style={{ width: '100%', maxWidth: '600px', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0', background: '#f8fafc', padding: '0.4rem' }}>
                                <img className="description-image" src={imgUrl} alt={t('products.descriptionImageAlt', { number: idx + 1 })} style={{ width: '100%', height: 'auto', display: 'block', borderRadius: '8px' }} />
                              </div>
                            ))}
                          </div>
                        )}
