...
const Navbar = ({ currentPage, setCurrentPage, cartItems = [], removeFromCart, updateCartQuantity, clearCart, searchQuery, setSearchQuery, customerAuth }) => {
  ...
  const [siteSettings, setSiteSettings] = useState({ store_name: 'مصنع الرحمة لتشكيل المعادن' });
  ...
  useEffect(() => {
    ...
    fetch('/api/settings')
      ...
      .then(data => data && setSiteSettings(prev => ({ ...prev, ...data })))
      .catch(() => {});
  ...
  return (
    <>
      <div className={`nav-wrapper ${currentPage === 'home' ? 'nav-transparent' : ''} ${isRahmaDarkPage ? 'nav-rahma-dark' : ''}`} style={{
        zIndex: 1000, 
        position: currentPage === 'home' ? 'absolute' : 'fixed',
        background: currentPage === 'home' ? 'transparent' : (isRahmaDarkPage ? 'rgba(12, 9, 7, 0.94)' : 'var(--bg-light)')
      }}>
        <nav className="navbar">
          <div className="flex justify-between items-center relative">
            ...
            {/* Icons */}
            <div className="nav-actions-container rahma-nav-actions" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <button className="clean-nav-btn rahma-nav-action show-on-mobile-only lang-btn-mobile" onClick={toggleLanguage}>
                  {language === 'ar' ? 'EN' : 'AR'}
                </button>

                <button className="clean-nav-btn rahma-nav-action mobile-menu-btn show-on-mobile-only" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
                ...
                <div className="nav-icons flex items-center" style={{ gap: '0.8rem' }}>
                  ...
                  <button className="clean-nav-btn rahma-nav-action hide-on-mobile" onClick={() => handleNavigate(customerAuth ? 'account' : 'login')} title={customerAuth ? t('nav.myAccount') : t('nav.login')}>
                    <User size={22} strokeWidth={2.5} />
                  </button>

                  <button className="clean-nav-btn rahma-nav-action rahma-lang-switch hide-on-mobile" onClick={toggleLanguage} style={{ fontWeight: '900', fontSize: '0.9rem' }}>
                    {language === 'ar' ? 'EN' : 'AR'}
                  </button>
                </div>
              </div>
