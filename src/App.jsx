...
import RahmaHomePage from './components/RahmaHomePage';
...
function App() { const { language, t } = useLanguage();
  ...
  const marketingPages = ['/software', '/branding'];
  ...
  useEffect(() => {
    ...
    try {
      const savedCart = localStorage.getItem('ElRahma_cart');
  ...
  useEffect(() => {
    if (cartItems.length > 0) {
      localStorage.setItem('ElRahma_cart', JSON.stringify(cartItems));
    } else {
      localStorage.removeItem('ElRahma_cart');
    }
  }, [cartItems]);
  ...
  const renderPage = () => {
    ...
    if (currentPage === 'store') {
      return (
        <div className="rahma-store-page">
          <section className="rahma-inner-page-hero rahma-store-hero">
            <span>{t('products.storeHeroEyebrow')}</span>
            <h1>{t('products.storeHeroTitle')}</h1>
            <p>{t('products.storeHeroDesc')}</p>
          </section>
    ...
    // Default home page
    return (
      <RahmaHomePage setCurrentPage={navigate} />
    );
  };
